...
class _ProductInTransitFormPageState extends ConsumerState<ProductInTransitFormPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_isEditing && _selectedProductTemplateId != null) {
        print('🔵 ProductInTransitFormPage: Загружаем атрибуты для редактирования товара...');
  ...
  Future<void> _loadTemplateAttributes() async {
    ...
    try {
      print('🔵 ProductInTransitFormPage: Загружаем атрибуты шаблона ID: $_selectedProductTemplateId');
      final templateDataSource = ref.read(productTemplateRemoteDataSourceProvider);
      ...
      print('🔵 ProductInTransitFormPage: Атрибуты шаблона загружены: ${_selectedTemplate!.attributes.length} шт');
      setState(() {});
    } catch (e) {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (_isEditing) {
        ...
        await ref.read(productsInTransitProvider.notifier).updateProduct(widget.product!.id, request);
      } else {
        // Создаем новый товар
        final request = CreateProductInTransitRequest(
          productTemplateId: _selectedProductTemplateId!,
        ...
        await ref.read(productsInTransitProvider.notifier).createProduct(request);
      }

      if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
            content: Text(_isEditing ? 'Товар успешно обновлен' : 'Товар успешно создан'),
