...
class InventoryStocksRemoteDataSourceImpl implements InventoryStocksRemoteDataSource {
  ...
  Future<InventoryStocksResponse> getStocks({
    ...
    try {
      ...
      // Дополнительная отладка для проверки поля correction_status
      if (response.data is Map<String, dynamic>) {
        ...
        if (data['data'] is List) {
          ...
          for (int i = 0; i < products.length && i < 3; i++) {
            ...
            print('🔍 correction_status в API: ${product['correction_status']}');
            print('🔍 correction_status тип: ${product['correction_status'].runtimeType}');
          }
        }
      }
